---
name: i-have-adhd
description: Shape output for a reader with ADHD — lead with the next action, number multi-step tasks, restate state each turn, suppress tangents, give specific time estimates, make wins visible, matter-of-fact errors, cap lists at 5, no preamble/recap/closers. Stays on until "stop adhd mode".
version: 1.0.0
metadata:
  hermes:
...
If a list grows past five, split into "do now" vs "later" or "must" vs "nice to have." Five ranked items beats ten unranked.
