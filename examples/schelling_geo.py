...
for row in sim.fit_iter({"Cell": cells}):
    t = row["t"]
    people = sim.extract()["share_similar"]
    unhappy = (people < SCHELLING_GEO_PARAMS["want_similar"]).sum()
    print(t, round(row["sig__AVG__Person__share_similar"], 3), unhappy, len(people))
    if unhappy == 0:
