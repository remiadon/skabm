...
- Language namespaces: `skabm.sparql` is SPARQL plumbing (prefixes, UDFs), `skabm.template` is the OTTR templates that map DataFrames into the graph (and `SCHEMA`, what each field means), `skabm.behaviour` is the rules, `skabm.dsl` is rules as SymPy compiled to SPARQL and JAX, `skabm.translate` generates DSL rules from a researcher's description and the OTTR templates.
